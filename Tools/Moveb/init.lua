...
MoveTool.ManualText = [[<font face="GothamBlack" size="24"><u><i>Move Tool  🛠</i></u></font>
...
function MoveTool:SetAxes(AxisMode)
	...
	if AxisMode == 'Global' then
		...
		if #Selection.Attachments > 0 and #Selection.Parts == 0 then
			...
			--		MoveTool.UIController:AttachmentsOnlyWarning()
		else
	...
	elseif AxisMode == 'Local' then
		self.HandleDragging:AttachHandles(Selection.Focus, true, false)
...
function MoveTool:UpdateFocus(AxisMode)
	
	if AxisMode == 'Global' then
		if #Selection.Attachments > 0 and Selection.Focus then
			
		--	self.HandleDragging:AttachHandles(Selection.Focus, nil, true)
			self.HandleDragging:AttachHandles(Selection.Focus, true, true)
		...
		elseif Selection.Focus then
			...
			BoundingBox.ClearBoundingBox();
				...
				local TheBoundingBox
			
				local _ = BoundingBox.StartBoundingBox(function (ABoundingBox)
					TheBoundingBox = BoundingBox.GetBoundingBox()
				end)
			
				repeat TheBoundingBox = BoundingBox.GetBoundingBox() print("crappy") task.wait(1 / 30) until TheBoundingBox or not self.Maid.AutofocusHandle
